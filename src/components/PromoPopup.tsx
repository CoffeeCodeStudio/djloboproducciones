    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) requestClose(onClose); }}>
      ...
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content
          ...
          tabIndex={-1}
          className={`fixed inset-0 m-auto w-[calc(100vw-1.5rem)] xs:w-[calc(100vw-2rem)] sm:w-auto max-w-[min(calc(100vw-1.5rem),28rem)] sm:max-w-md h-fit max-h-[80vh] sm:max-h-[85vh] grid gap-0 p-0 overflow-hidden flex flex-col glass-card border-2 border-primary/60 promo-neon-glow focus:outline-none ${closing ? "promo-popup-exit-centered" : "promo-popup-enter-centered"}`}
          style={{
          ...
          onEscapeKeyDown={(e) => {
            ...
            // EXIT_ANIM_MS animation before the dialog actually unmounts.
            e.preventDefault();
            requestClose(onClose);
          }}
          onOpenAutoFocus={(e) => {
            // Move initial focus onto the dialog container itself instead of
            // the first focusable child (often the close button), so screen
            // readers announce the dialog before announcing "Stäng".
            e.preventDefault();
            // Defer one tick so the content node is fully mounted/visible.
